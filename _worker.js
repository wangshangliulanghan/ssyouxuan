// Cloudflare Worker - 简化版优选工具 (纯净私库极简命名版 + 极限网速优化版)
// 修复记录：重写 IPv6 解析、关闭默认杂质源、彻底精简生成节点名称后缀。
// 提速记录：强力注入 0-RTT (Early Data)、强制 ALPN h2 并发、开启 TFO (TCP Fast Open) 与 UDP 支持。
    ...
    list.forEach(item => {
        ...
        portsToGenerate.forEach(({ port, tls }) => {
            ...
            if (tls) {
                ...
                // 🚀 核心提速 1：强制开启 ALPN h2，利用 HTTP/2 的多路复用降低高并发时的延迟
            ...
            if (protocols.evEnabled) {
                ...
                // 🚀 核心提速 2：注入 ed=2048 (Early Data)，省去一个 RTT 的握手时间，实现秒连
                if (tls) vlessParams.set('ed', '2048');
...
function generateClashConfig(links) {
    // 🚀 核心提速 3：全局开启 TCP Fast Open 和 IPv6 优先
    let yaml = 'port: 7890\nsocks-port: 7891\nallow-lan: false\nmode: rule\nlog-level: info\nipv6: true\ntcp-fast-open: true\n\nproxies:\n';
    const proxyNames = [];
...
function generateSurgeConfig(links) {
    ...
    links.forEach((link, i) => {
        ...
        // 🚀 核心提速 4：注入 tfo=true, udp-relay=true 提升 Surge 客户端的并发处理能力
        config += `${name} = vless, ${link.match(/@([^:]+):(\d+)/)?.[1] || ''}, ${link.match(/@[^:]+:(\d+)/)?.[1] || '443'}, username=${link.match(/vless:\/\/([^@]+)@/)?.[1] || ''}, tls=${link.includes('security=tls')}, ws=true, ws-path=${link.match(/path=([^&#]+)/)?.[1] || '/'}, ws-headers=Host:${link.match(/host=([^&#]+)/)?.[1] || ''}, tfo=true, udp-relay=true\n`;
    });
