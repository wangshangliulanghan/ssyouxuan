// Cloudflare Worker - 极限性能版优选工具 (v4.3 兼容优化版：新增 UDP 控制与 Karing/Sing-box 防报错降级)
...
function generateNodesFromList(list, user, workerDomain, disableNonTLS, customPath, echConfig, protocols, config) {
    ...
    // 动态生成 ALPN (解决 Karing/Sing-box 报错核心逻辑)
    let alpnStr = 'h2,http/1.1'; // 默认关闭 UDP 时的回退
    if (config.strictH1) {
        alpnStr = 'http/1.1'; // Karing 兼容模式：强制降级 HTTP/1.1 彻底切断 HTTP/2 流报错
    } else if (config.enableUDP) {
        alpnStr = 'h3,h2,http/1.1'; // 开启 UDP 才允许尝试 HTTP/3
    }
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .header h1 { font-size: 36px; font-weight: 700; letter-spacing: -0.3px; color: #1d1d1f; margin-bottom: 8px; line-height: 1.1; }
        .header p { font-size: 15px; color: #86868b; font-weight: 400; line-height: 1.5; }
        ...
        .list-item-label { font-size: 16px; font-weight: 500; color: #1d1d1f; flex: 1; }
        .list-item-description { font-size: 12px; color: #86868b; margin-top: 4px; line-height: 1.4; }
        ...
        .btn { width: 100%; padding: 16px; font-size: 17px; font-weight: 600; color: #ffffff; background: #007AFF; border: none; border-radius: 14px; cursor: pointer; transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1); margin-top: 8px; -webkit-appearance: none; box-shadow: 0 4px 12px rgba(0, 122, 255, 0.25); }
        .client-btn { padding: 12px 16px; font-size: 14px; font-weight: 500; color: #007AFF; background: rgba(0, 122, 255, 0.1); border: 1px solid rgba(0, 122, 255, 0.2); border-radius: 12px; cursor: pointer; transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; min-width: 0; }
        .client-btn:active { transform: scale(0.97); background: rgba(0, 122, 255, 0.2); }
        .checkbox-label { display: flex; align-items: center; cursor: pointer; font-size: 16px; font-weight: 400; user-select: none; padding: 8px 0; }
        .checkbox-label input[type="checkbox"] { margin-right: 12px; width: 22px; height: 22px; cursor: pointer; flex-shrink: 0; }
        .section-title { font-size: 14px; font-weight: 600; color: #007AFF; margin: 32px 0 16px; text-transform: uppercase; letter-spacing: 1px; border-bottom: 1px solid rgba(0, 122, 255, 0.2); padding-bottom: 8px; }
        @media (prefers-color-scheme: dark) {
            body { background: linear-gradient(180deg, #000000 0%, #1c1c1e 50%, #2c2c2e 100%); color: #f5f5f7; }
            .card { background: rgba(28, 28, 30, 0.75); border: 0.5px solid rgba(255, 255, 255, 0.12); box-shadow: 0 4px 24px rgba(0, 0, 0, 0.3); }
...
<body>
    <div class="container">
        <div class="header">
            <h1>优选工具 <span style="color:#007AFF; font-size: 20px; vertical-align: super;">v4.3</span></h1>
            <p>XHTTP 与 Karing 兼容优化防断流版</p>
        </div>
        ...
        <div class="card">
            ...
            <div class="form-group">
                <label>请求路径（WS / XHTTP）</label>
                <input type="text" id="customPath" placeholder="留空则使用默认路径 /" value="/">
            </div>
            
            <div class="section-title">数据源与过滤</div>
            ...
            <div class="form-group" style="margin-top: 12px;">
                <input type="text" id="githubUrl" placeholder="私有库URL(留空用内置纯净库)" style="font-size: 14px;">
            </div>
            <div class="form-group">
                <label>运营商选择</label>
            ...
            <div class="section-title">传输协议控制</div>
            <div style="display: flex; gap: 16px; margin-bottom: 16px;">
                <label class="checkbox-label"><input type="checkbox" id="switchVL" checked><span>VLESS</span></label>
                <label class="checkbox-label"><input type="checkbox" id="switchTJ"><span>Trojan</span></label>
                <label class="checkbox-label"><input type="checkbox" id="switchVM"><span>VMess</span></label>
            </div>
            
            <div class="list-item" onclick="toggleSwitch('switchXHTTP')">
                <div>
                    <div class="list-item-label">启用 XHTTP 协议</div>
                    <div class="list-item-description">替代 WS 传输 (未开启则默认使用 WS)</div>
                </div>
            ...
            <div class="list-item" onclick="toggleSwitch('switchUDP')">
                <div>
                    <div class="list-item-label" style="color: #ff9500;">启用 UDP 转发 (HTTP/3)</div>
                    <div class="list-item-description">若被运营商阻断会导致断网，建议默认关闭</div>
                </div>
                <div class="switch" id="switchUDP"></div>
            </div>
            
            <div class="list-item" onclick="toggleSwitch('switchH1')">
                <div>
                    <div class="list-item-label" style="color: #ff3b30;">Karing / Sing-box 兼容模式</div>
                    <div class="list-item-description">强降 HTTP/1.1，彻底解决 END_STREAM 报错</div>
                </div>
                <div class="switch" id="switchH1"></div>
            </div>

            <div class="list-item" onclick="toggleSwitch('switchTLS')">
                <div><div class="list-item-label">仅生成 TLS 节点</div></div>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); gap: 10px;">
                ...
                <button type="button" class="client-btn" onclick="generateClientLink('v2ray', 'V2RAY / Karing')">V2RAY / Karing</button>
            ...
            <div id="clientSubscriptionUrl" style="display: none; margin-top: 16px; padding: 12px; background: rgba(0, 122, 255, 0.1); border-radius: 8px; font-size: 13px; color: #007aff; word-break: break-all;"></div>
        </div>
    </div>
    
    <script>
        let switches = {
            switchDomain: false, switchIP: false, switchGitHub: true, 
            switchXHTTP: false, switchUDP: false, switchH1: false, switchTLS: false
        };
        ...
        function toggleSwitch(id) {
            ...
            if (id === 'switchH1' && switches.switchH1) {
                if (switches.switchUDP) toggleSwitch('switchUDP'); // 兼容模式自动关UDP
            }
            if (id === 'switchUDP' && switches.switchUDP) {
                if (switches.switchH1) toggleSwitch('switchH1'); // 开UDP自动关兼容模式
            }
        }
        ...
        function generateClientLink(clientType, clientName) {
            ...
            const vlessEnabled = document.getElementById('switchVL').checked;
            const trojanEnabled = document.getElementById('switchTJ').checked;
            const vmessEnabled = document.getElementById('switchVM').checked;
            if (!vlessEnabled && !trojanEnabled && !vmessEnabled) return alert('请至少选择一个代理协议');
            ...
            if (vlessEnabled) subUrl += '&ev=yes';
            if (trojanEnabled) subUrl += '&et=yes';
            if (vmessEnabled) subUrl += '&mess=yes';
            ...
            if (clientType === 'v2ray') {
                ...
                finalUrl = SUB_CONVERTER_URL + '?target=' + clientType + '&url=' + encodeURIComponent(subUrl) + '&insert=false&emoji=true&list=false&xudp=' + (switches.switchUDP ? 'true' : 'false') + '&udp=' + (switches.switchUDP ? 'true' : 'false') + '&new_name=true';
                document.getElementById('clientSubscriptionUrl').textContent = finalUrl;
                document.getElementById('clientSubscriptionUrl').style.display = 'block';
                navigator.clipboard.writeText(finalUrl).then(() => alert(clientName + ' 订阅链接已复制'));
            }
