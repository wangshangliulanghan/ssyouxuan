// Cloudflare Worker - 简化版优选工具 (随机多端口优化版 & 最大兼容版：禁用h3/alpn)
// 仅保留优选域名、优选IP、GitHub、上报和节点生成功能
// 优化记录：引入随机分配 CF 冷门端口机制，缓解 443 晚高峰 QoS
// 修复记录：修复页面 JS 模板字符串导致 Uncaught SyntaxError: Unexpected identifier '$' 报错
// 兼容记录（方案三）：移除所有 alpn=h3... 写入，避免部分客户端/环境 TLS 协商异常
  ...
  list.forEach(item => {
    ...
    if (item.port) {
      ...
      if (CF_HTTPS_PORTS.includes(port)) {
        ...
        if (!disableNonTLS) portsToGenerate.push({ port: port, tls: false });
      } else {
        portsToGenerate.push({ port: port, tls: true });
      }
    } else {
...
function generateVMessLinksFromSource(list, user, workerDomain, disableNonTLS = false, customPath = '/', echConfig = null) {
  ...
  list.forEach(item => {
    ...
    portsToGenerate.forEach(({ port, tls }) => {
      ...
      const vmessBase64 = btoa(encodeURIComponent(jsonStr).replace(/%([0-9A-F]{2})/g, function toSolidBytes(match, p1) {
...
// 从GitHub IP生成链接（最大兼容：ech 不追加 alpn）
function generateLinksFromNewIPs(list, user, workerDomain, customPath = '/', echConfig = null) {
  ...
  // 方案三：echSuffix 不再包含 alpn
  const echSuffix = echConfig ? `&ech=${encodeURIComponent(echConfig)}` : '';

  list.forEach(item => {
    const nodeName = (item.name || item.ip).replace(/\s/g, '_');
...
async function handleSubscriptionRequest(
  ...
  const finalLinks = [];
  const workerDomain = url.hostname;              // 请求的 hostname（Worker 域名）
  const nodeDomain = customDomain || url.hostname; // 生成节点 Host/SNI 用的域名（你传的 domain=）
...
// 生成Quantumult配置（保持原样：base64）
function generateQuantumultConfig(links) {
  return btoa(links.join('\n'));
}

// 生成iOS风格的主页（保持原样，修复模板字符串注入）
...
function generateClientLink(clientType, clientName) {
  ...
  if (clientType === 'v2ray') {
    ...
    if (clientName === 'V2RAY') {
      navigator.clipboard.writeText(finalUrl).then(() => alert(displayName + ' 订阅链接已复制'));
    } else if (clientName === 'Shadowrocket') {
      schemeUrl = 'shadowrocket://add/' + encodeURIComponent(finalUrl);
      tryOpenApp(schemeUrl, () => navigator.clipboard.writeText(finalUrl).then(() => alert(displayName + ' 订阅链接已复制')));
